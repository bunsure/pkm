...
class DownloadController extends Controller
{
     ...
     function get_record($id_dokumen){
        $record = DB::table('download as a')->where('id_dokumen', $id_dokumen)->first();
        if($record){
     ...
     function dt_download(){
        $query = DB::table('download as a')
        ...
        ->addColumn('action', function ($query) {
         ...
         ->addColumn('nama', function ($query) {
            ...
            if($query->tipe=='file'){
                $download = '<a href="https://docs.google.com/viewer?url='.$link.'" target="_top">'.$query->nama.'</a>';
            }else{
                $download = '<a href="'.$link.'" target="_top">'.$query->nama.'</a>';
            }
            return $download;
        })
        ->addIndexColumn()
     ...
     function insert(Request $request){
        ...
        if ($validator->passes()) {
            ...
            DB::table('download')->insert($record);
     ...
     function update(Request $request){
        ...
        if ($validator->passes()) {
            ...
            $record = array( 
                "nama"=>$request->nama);
            DB::table('download')->where('id_dokumen',$request->id_dokumen)->update($record);
     ...
     function delete(Request $request){
        ...
        if ($validator->passes()) {
            ...
            $dokumen = DB::table('download')->where('id_dokumen', $request->id_dokumen)->first();
            DB::table('download')->where('id_dokumen',$request->id_dokumen)->delete();
